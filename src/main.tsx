...
const _getPreviewRequests = (): readonly CancelRequest[] => {
  const now = Math.floor(Date.now() / 1000);
  const mk = (
    username: string,
    fullName: string,
    status: CancelRequest["status"],
    extra: Partial<CancelRequest> = {},
  ): CancelRequest => ({
    username,
    id: status === "unverified" ? null : `${username}-id`,
    timestamp: now - 86400 * Math.floor(Math.random() * 120),
    status,
    checked: status !== "unverified",
    selected: status === "pending" || status === "following",
    fullName: status === "unverified" ? undefined : fullName,
    profilePicUrl: status === "unverified" ? undefined : _avatarUrl(username),
    ...extra,
  });
  return [
    mk("alina.frames", "Alina Moreno", "pending", { isVerified: true }),
    mk("brassandbone", "Theo Walsh", "pending", { isPrivate: true }),
    mk("citrus.archive", "Mara Kim", "following"),
    mk("dawnledger", "Jon Bell", "following", { isPrivate: true }),
    mk("elias.market", "Elias Noor", "accepted"),
    mk("fieldnotes.studio", "Nadia Reyes", "done"),
    mk("glint.supply", "Remy Park", "failed"),
    mk("harbor.sequence", "Ivy Chen", "ghost"),
    mk("inkline.daily", "", "unverified"),
    mk("juniper.signal", "", "unverified"),
    mk("keystone.labs", "", "unverified"),
  ];
};
...
function App() {
  const previewParam = isLocalPreview ? new URLSearchParams(location.search).get("preview") : null;
  const [state, setState] = useState<State>(
    previewParam === "scanning"
      ? {
        status: "scanning",
        page: 1,
        searchTerm: "",
      ...
      : previewParam === "cancel"
        ? { status: "cancelling", requests: _getPreviewRequests() }
  ...
  switch (state.status) {
    ...
    case "unfollowing":
      ...
      break;
    case "cancelling":
      // The CancelRequests component manages its own active-process / beforeunload guard.
      isActiveProcess = false;
      break;
    default:
  ...
  switch (state.status) {
    ...
    case "cancelling":
      // Self-contained flow with its own header; rendered without the standard Toolbar.
  ...
  return (
    <main id="main" role="main" className="iu">
      <section className="overlay">
        {/* The cancel flow renders its own header, so the standard Toolbar is hidden for it. */}
